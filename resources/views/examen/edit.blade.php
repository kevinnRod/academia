...
@section('titulo')
    <h3 class="page-title mb-0 p-0">Editar Examen</h3>
@endsection

@section('rutalink')
    <li class="breadcrumb-item" aria-current="page"><a href="{{ route('examen.index') }}">Exámenes</a></li>
    <li class="breadcrumb-item active" aria-current="page">Editar Examen</li>
@endsection
    ...
    <div class="row text-black">
        <div class="col-sm-12">
            <div class="card border-primary shadow-sm">
                <div class="card-body">
                    <form method="POST" action="{{ route('examen.update', $examen->idexamen) }}">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label for="descripcion" class="form-label">Descripción</label>
                            <input type="text" class="form-control" id="descripcion" name="descripcion" value="{{ old('descripcion', $examen->descripcion) }}">
                            @error('descripcion')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="fecha" name="fecha" value="{{ old('fecha', $examen->fecha) }}">
                            @error('fecha')
                                <div class="alert alert-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" id="idperiodo" name="idperiodo" onchange="cargarTiposCiclo()">
                                    ...
                                    <option value="{{ $periodo->idperiodo }}" {{ $periodo->idperiodo == $examen->aula->ciclo->idperiodo ? 'selected' : '' }}>
                                        {{ $periodo->idperiodo }}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" id="idtipociclo" name="idtipociclo" onchange="cargarAreas()">
                                    ...
                                    @foreach ($tiposCiclo as $tipo)
                                        <option value="{{ $tipo->idtipociclo }}" {{ $tipo->idtipociclo == $examen->aula->ciclo->idtipociclo ? 'selected' : '' }}>
                                            {{ $tipo->descripcion }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" id="idarea" name="idarea" onchange="cargarCiclos()">
                                    ...
                                    @foreach ($areas as $area)
                                        <option value="{{ $area->idarea }}" {{ $area->idarea == $examen->aula->ciclo->idarea ? 'selected' : '' }}>
                                            {{ $area->descripcion }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" id="idciclo" name="idciclo" onchange="cargarAulas()">
                                    ...
                                    @foreach ($ciclos as $ciclo)
                                        <option value="{{ $ciclo->idciclo }}" {{ $ciclo->idciclo == $examen->aula->ciclo->idciclo ? 'selected' : '' }}>
                                            {{ $ciclo->descripcion }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="idtipoexamen" name="idtipoexamen">
                                ...
                                @foreach ($tiposExamen as $tipoExamen)
                                    <option value="{{ $tipoExamen->idtipoexamen }}" {{ old('idtipoexamen', $examen->idtipoexamen) == $tipoExamen->idtipoexamen ? 'selected' : '' }}>
                            ...
                            <label for="idaula" class="form-label">Aula</label>
                                    <select name="idaula" id="idaula" class="form-select">
                                        ...
                                        @foreach ($aulas as $aula)
                                            <option value="{{ $aula->idaula }}" {{ $aula->idaula == $examen->idaula ? 'selected' : '' }}>
                                                {{ $aula->descripcion }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="estado" name="estado">
                                <option value="1" {{ old('estado', $examen->estado) == '1' ? 'selected' : '' }}>Activo</option>
                                <option value="0" {{ old('estado', $examen->estado) == '0' ? 'selected' : '' }}>Inactivo</option>
                            </select>
                            @error('estado')
                                <div class="alert alert-danger mt-2">{{ $message }}</div>
                            @enderror
